...
>the rating of each mountain, and the name of the mountain.
These help us fit the moutians into categories

b.  What about a visualization of the *quantitative* `elevation` variable?

>A quantitative visualzation would be like the elevation, and the time it takes to climb the mountians.
...
>8 of them are easy, 11 of them are medium and 26 of them are difficult

2.  Are the hikes evenly distributed among these categories, or are some more common than others?

>there are more difficult mountains and less easier mountains.
...
```{r}
# the bars are grey, and thet are sorted by difficultly, the number of hikes is the y value
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# this one has the same "number of hikes" but now the bars are blue with the fill = blue. 
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# the code "color" made the outline of the bats orange, and the fill was still blue
ggplot(hikes, aes(x = rating)) +
...
```{r}
# theme "theme_minimal" made the grey background go away. 
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
>to continue what you want to do in your code easier

- We added the bars using `geom_bar()`? Why "geom"?

>geometrical bar and it adds layers to make a visual graph

- What does `labs()` stand for?

>this stands for labels on the graph 

- What's the difference between `color` and `fill`?

>color is the boarder color of the graph and fill is the filled in color of the visual
...
>the categories we observed are: diffucult easy and moderate

- **variability between categories**: Are observations evenly spread out among the categories, or are some categories more common than others?

>no, the observations are not evenly distributed as there tend to be more moderatly rated hikes as shown, and there are less dated as diffucult and easy. 
...
>From the graph, we can learn that there are more then 25 people who rated their hikes as moderate, and there are less then half of that amount of people who rated the hike as easy. And a lower number of people who rated the hike as diffucult.  
...
>the x axsis is not in order from easy, moderate, difficult. To make the graph more conscise it would be better to put them in order. 
...
>this might not be a great graph to look at because its hard to desipsher where the averages of the hikes are. It makes it diffucult to answer the question and to find the range of how the hikes are distributed. 
...
> approximately 13 hikes have an elevation between 4500 and 4700ft  

- How many total hikes have an elevation of at least 5100 feet?

> around 4 hikes have an elevation of at least 5100ft
...
```{r}
# this is the normal grpah, how its set to be
ggplot(hikes, aes(x = elevation)) +
  geom_histogram()
```

```{r}
# this graph has changed because the color is now white, which has made them more distict from each other 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") 
```

```{r}
# This changes the grapph so that the white is the outline and the blue is the fill color
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white", fill = "blue") 
```

```{r}
# this graph has changed so that the outline is still white, but the x axis to show the elvation and the number of hikes. 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") +
...
```{r}
# this shows that the, binwidth has been changed and that effects the width of the graph and the chart  
ggplot(hikes, aes(x = elevation)) +
...
```{r}
# this looks like the "sad" plot we saw, and this makes it so that the bin width is smaller
ggplot(hikes, aes(x = elevation)) +
...
```{r}
# okay this shows what would make a normal width for the graphs! around 200
ggplot(hikes, aes(x = elevation)) +
